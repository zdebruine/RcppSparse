...
class cscMatrix {
   public:
    ...
    // constructors
    cscMatrix(Rcpp::NumericVector x, Rcpp::IntegerVector i, Rcpp::IntegerVector p, Rcpp::IntegerVector Dim) : x(x), i(i), p(p), Dim(Dim) {}
    cscMatrix(const Rcpp::S4& s) {
    ...
    cscMatrix clone() {
        ...
        return cscMatrix(x_, i_, p_, Dim_);
    }
...
#pragma omp parallel for
    ...
    class InnerIterator {
       public:
        InnerIterator(cscMatrix& ptr, int col) : ptr(ptr), col_(col), index(ptr.p[col]), max_index(ptr.p[col + 1]) {}
    ...
    class InnerIteratorInRange {
       public:
        InnerIteratorInRange(cscMatrix& ptr, int col, std::vector<unsigned int>& s) : ptr(ptr), s(s), col_(col), index(ptr.p[col]), max_index(ptr.p[col + 1] - 1), s_max_index(s.size() - 1) {
    ...
    class InnerIteratorNotInRange {
       public:
        InnerIteratorNotInRange(cscMatrix& ptr, int col, std::vector<unsigned int>& s_) : ptr(ptr), col_(col), index(ptr.p[col]), max_index(ptr.p[col + 1] - 1) {
    ...
    class InnerRowIterator {
       public:
        InnerRowIterator(cscMatrix& ptr, int j) : ptr(ptr) {
